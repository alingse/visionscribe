...
def slice(input_file: Path, output_dir: Path, fps: int, dedup: bool) -> None:
    """Slice video into frames with timestamp info.
    
    Stage 1: Slice video into frames and save as timestamped images.
    ...
    Example: visionscribe slice tutorial.mp4 ./output-dir
    """
    try:
        video_processor = VideoProcessor()
        click.echo(f"🎬 Slicing frames from {input_file} at {fps} FPS...")
        ...
        click.echo(f"✅ 共切片了 {len(frames)} 个图片")
...
@click.option("--output", "-o", default="./text_data.json", help="Output JSON file", type=click.Path(path_type=Path))
@click.option("--languages", default="en,zh", help="Text recognition languages")
@click.option("--confidence", default=0.8, type=float, help="Minimum confidence threshold")
def read(frames_dir: Path, output: Path, languages: str, confidence: float) -> None:
    """Read text from frames and create structured JSON.
    
    Stage 2: Read text from extracted frames and create comprehensive JSON data.
    ...
    Example: visionscribe read ./extracted-frames ./text-output.json
    """
    try:
        import cv2
        
        text_extractor = TextExtractor(languages=languages.split(","))
        click.echo(f"📖 Reading frames in {frames_dir}...")
        ...
        # Process frames with text reading
        text_data = []
        for img_path, frame in frames:
            ...
            text_data.append(frame_data)
        
        # Save comprehensive text data
        with open(output, 'w', encoding='utf-8') as f:
            json.dump({
                "total_frames": len(text_data),
                "processing_info": {
        ...
        click.echo(f"✅ Text data saved to {output}")
        click.echo(f"📊 Processed {len(text_data)} frames with text")
...
def analyze(input_json: Path, output: Path, model: str, openai_api_key: Optional[str], dedup: bool) -> None:
    """Analyze and deduplicate text data using AI.
    
    Stage 3: AI-powered analysis and deduplication of extracted text data.
    
    This command takes the comprehensive text JSON data, uses AI to analyze content,
    remove duplicates, and create a refined structured dataset for project reconstruction.
    
    Example: visionscribe analyze ./text-data.json ./analyzed-data.json
    """
    try:
        import json
        
        click.echo(f"🧠 Loading text data from {input_json}...")
        
        # Load text data
        with open(input_json, 'r', encoding='utf-8') as f:
            text_data = json.load(f)
        
        click.echo(f"📊 Loaded {text_data.get('total_frames', 0)} frames")
        
        # Extract all text data
        all_texts = []
        for frame in text_data.get('frames', []):
        ...
        analysis_result = {
            "total_original_texts": len(all_texts) if 'all_texts' in locals() else text_data.get('total_frames', 0),
            "analysis_metadata": {
                "model_used": model,
                "ai_provider": "openai",
                "created_at": str(Path().cwd()),
...
@click.option("--model", default="gpt-4", help="AI model to use")
@click.option("--openai-api-key", help="OpenAI API key")
def rebuild(input_json: Path, output_dir: Path, format: str, model: str, openai_api_key: Optional[str]) -> None:
    """Rebuild project files from analyzed JSON data.
    
    Stage 4: Rebuild actual project files from refined JSON analysis.
    ...
    Example: visionscribe rebuild ./ai-analysis.json ./final-project
    """
    try:
        import json
        
        click.echo(f"🏗️  Rebuilding project from {input_json}...")
        ...
        summary_file = output_dir / "rebuild_summary.json"
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2)
        
        click.echo(f"🎉 Project rebuilding completed!")
        click.echo(f"📁 Output directory: {output_dir}")
        click.echo(f"📊 Rebuild summary: {summary_file}")
...
@click.option("--model", default="gpt-4", help="AI model to use")
@click.option("--openai-api-key", help="OpenAI API key")
def extract(input_file: Path, output_dir: Path, format: str, fps: int, 
         languages: str, model: str, openai_api_key: Optional[str]) -> None:
    """Extract complete project from video in one command.
    
    This is the main command for complete video processing. Just specify
    the input video file and output directory, and VisionScribe will:
    1. Slice video into frames
    2. Read text from frames
    3. Analyze content with AI
    4. Rebuild project structure
    5. Generate code and/or documentation
    
    Example: visionscribe extract tutorial.mp4 ./project-output
    """
    try:
        ...
        # Process video
        click.echo("🎬 Slicing frames...")
        frames = video_processor.extract_frames(str(input_file), fps)
        click.echo(f"✅ Sliced {len(frames)} frames")
        
        # Read text
        click.echo("📖 Reading text...")
        text_blocks = text_extractor.extract_text_from_video(frames)
        click.echo(f"✅ Read {len(text_blocks)} text blocks")
        ...
        click.echo("✅ Content analysis completed")
        
        # Rebuild project
        if format in ["code", "both"]:
            click.echo("💻 Rebuilding code...")
            code_output = output_dir / "code"
            output_generator.generate_codebase(project_data, str(code_output))
            click.echo(f"✅ Code rebuilt in {code_output}")
        
        if format in ["docs", "both"]:
            click.echo("📄 Rebuilding documentation...")
            doc_output = output_dir / "docs.md"
            output_generator.generate_documentation(project_data, str(doc_output))
            click.echo(f"✅ Documentation rebuilt: {doc_output}")
        
        click.echo(f"🎉 Extraction completed! Check {output_dir} for results.")
